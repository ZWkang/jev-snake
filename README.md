...
顶栏的“反馈”进入 `/feedback`。游客无需登录，投稿成功即公开，显示“匿名用户”、正文和时间；HTML 按纯文本展示。页面提醒不要填写个人资料或密钥，正文不会被程序自动匿名化。`/watch?admin=1` 解锁后可在社区管理中隐藏或恢复反馈，隐藏后公开 API 不再返回该内容。

“企微交流群”展示配置的真实入群链接或二维码。未配置时明确显示“交流群入口暂未配置”；本站只提供入群指引，不把跳转当作已经入群。配置尚未提供时也可使用反馈功能。

“贡献 Key”支持 Typesafe / OpenRouter，提交前需要主动确认：验证会发起一次真实 JEV 调用，成功后自动保存并启用，持续观战可能继续产生费用。通过验证并成功入库后才展示“谢谢你的支持，让小蛇继续向前！”及一次约 650ms 的感谢动效；重复回执不重播，减少动态效果时静态显示，感谢文案持续保留。

贡献 Key 仅以 AES-256-GCM 密文保存；独立主密钥不得放入数据库、Git 或前端配置。管理员只看到掩码及状态，无法读取完整 Key。本人可在同一弹窗选择撤回并输入原 Key，撤回不调用服务商，清除活动密文并阻止后续观战请求。已发出的调用仍可能完成并计费，历史备份中的旧密文不承诺物理删除。生产须使用 HTTPS，并将 `WATCH_PUBLIC_ORIGIN` 设为实际 HTTPS Origin。

| 配置 | 默认值 | 行为 |
| --- | --- | --- |
| `COMMUNITY_WECOM_JOIN_URL` | 空 | HTTPS 入群链接 |
| `COMMUNITY_WECOM_QR_URL` | 空 | 同源图片路径或 HTTPS 二维码图片 |
| `COMMUNITY_MAX_BODY_BYTES` | `16384` | 公共及社区管理 JSON 请求体大小限制；0 显式关闭，超过时返回 413，不截断正文 |
| `JEV_CONTRIBUTIONS_ENABLED` | `false` | 设 true 开放新贡献；关闭后仍可撤回已有贡献、管理员停用 |
| `JEV_CREDENTIAL_POOL` | `false` | 设 true 后连续观战实际使用同服务商凭证池；false 保持环境单 Key 行为 |
| `CREDENTIALS_MASTER_KEY` | 空 | 任一开关启用时必填，32 字节 base64 秘密；用 `openssl rand -base64 32` 生成并安全保管 |

修改配置后重启相关服务。已有 `.env` 按需添加配置，不用示例覆盖。开启贡献入口不等于自动开启频道；开启池模式但缺少或无法使用加密配置时明确报错。

凭证池只使用 `JEV_PROVIDER` 选定服务商及当前目标模型；非当前服务商的贡献按它的默认模型验证，保存后暂不调度。模型配置改变后，贡献需针对新目标重新验证。站长环境 Key 与贡献按稳定顺序逐局轮换，局内固定当前 Key；验证调用不占轮次。没有环境 Key 但有匹配贡献时也可运行。CLI 继续使用显式环境单 Key，不自动使用共享池。

只有可确认的凭证失效或余额耗尽才排除当前 Key，并记录失败后选择同服务商的下一把；不会切换服务商或模型。Typesafe 当前仅把明确 401 归为凭证失效，未公开的额度错误仍暂停。OpenRouter 明确的本层 401，以及可确认余额耗尽的 402 可轮换；带 `Retry-After` 或临时预算元数据的 402、429、403、网络、格式和未知错误均暂停，不轮换规避限制。所有 Key 都不可用时记录“无可用凭证”故障；新贡献不清除已保存故障，管理员需明确恢复。相同环境 Key 完成一次明确的成功贡献验证后，也会更新该环境凭证的可用状态。

停止后续开局与立即停止本局仍遵循原有语义。重启恢复同一未完成响应局及其凭证绑定，不推进局间轮换游标。验证或上游调用开始后异常退出而无完成记录时，标为“结果未确认”，不自动重新计费请求；显式重新验证可能再次产生费用。模型选择、实际 HTTP 正文和公开回放不包含私有凭证标识。

管理面板分别显示验证、是否启用、是否匹配当前频道和实际观战调用；缺少 usage/费用不补零，调用次数也不代表均等费用分摊。隐私范围是业务表和公开展示，网络代理仍可能有自身访问日志。

新增 API：

- `GET /api/community`：公开配置；`GET/POST /api/feedback`：读取可见反馈及匿名投稿。
- `POST /api/key-contributions`：授权验证与贡献；`POST /api/key-contributions/status`：凭高熵 requestId 查询本次回执；`POST /api/key-contributions/revoke`：持原 Key 撤回。
- `GET /api/watch-admin/feedback`、`POST /api/watch-admin/feedback/:id/visibility`：反馈管理。
- `GET /api/watch-admin/credentials`、`POST /api/watch-admin/credentials/:id/commands`：脱敏列表及明确的停用/重新验证命令。

新公共写接口校验精确 Origin、输入和请求标识，只能投稿或管理本人贡献，不授予游戏控制或管理员权限。管理接口沿用现有 HttpOnly 会话及 `/api/watch-admin` cookie 路径。

社区功能将数据库从 v2 原子迁移到 v3，保留对局、事件、频道与旧回执。部署前停写并做一致性备份，同时安全备份独立主密钥。常规回退可关闭两个开关并保留 v3 兼容后端；必须使用 v2 旧程序时需停写并明确恢复升级前备份，备份后的数据不会包含在旧库中。不能直接降版本、删除新表或把解密失败当作空凭证池。
...
游戏观战、历史与回放接口公开只读；匿名反馈与自愿 Key 贡献使用上文列出的独立公共写接口。游戏控制和管理写操作仍在服务端校验权限，密钥不进入网页构建、URL 或公开事件。
