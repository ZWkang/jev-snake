...
连续频道同时关闭 `enabled` 并进入带“费用保护”说明的暂停状态，**不会倒计时换下一局继续花钱**。重启保留暂停，只有管理员明确恢复才新开局；即使进程在“本局暂停已保存、频道状态尚未保存”之间退出，恢复时也会识别保护原因并保持关闭。
...
实时入口使用 `growth-space-v15`。保留完整字符图、身体顺序、静态事实和历史，在 V14 的动态分析上修复“吃果时停止检查”的缺口。JEV 仍从本步合法方向中选择，每次只执行其原始响应。完整字段见 [V15 说明](docs/jev-growth-space.md)。
...
- 当前苹果候选最多搜索 32 步，每个增长端点再检查吃果后的 8 步，而非只数即时出口。已证明吃后困死的到达方式会被拒绝，继续搜索不同身体排列的到达方式；这些拒绝不会移除模型的方向选项。
...
- `JEV_DYNAMIC_ANALYSIS=false` 显式切回 V13；默认启用 V15。非法配置直接报错。没有自动代选、超时直行或隐藏重试，原始概率继续保留；无进展时由独立的费用保护暂停调用。
