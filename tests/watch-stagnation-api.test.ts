...
test.each([
	...
	"cost protection stops before the third model call and remains paused across restart (before threshold=%s, completion commit gap=%s)",
	async (restartBeforeThreshold, completionCommitGap) => {
		const directory = mkdtempSync(join(tmpdir(), "snake-watch-stagnation-"));
		const transport = globalThis.fetch;
		const calls: DecisionRequestV15[] = [];
		...
		vi.stubGlobal(
			...
			async (input: Parameters<typeof fetch>[0], init?: RequestInit) => {
				...
				calls.push(body);
				if (holdSecondMove && body.state.timing.observedTick === 1) {
		...
		try {
			...
			expect(protectedState).toMatchObject({
				...
				nextStartAt: null,
				error: {
					code: "stagnation_no_apple",
					message: stagnationMessage("stagnation_no_apple"),
				},
			});
			...
			expect(replay.state).toMatchObject({ enabled: false, phase: "fault" });
			expect(calls).toHaveLength(beforeNextRound);
			expect(
				(
					await post(
						"commands",
						{ requestId: "enable-new-round", enabled: true },
						cookie,
					)
				).status,
			).toBe(200);
			await vi.waitFor(
				() => {
					expect(game.channel.snapshot().phase).toBe("fault");
					expect(game.channel.snapshot().lastMatchId).not.toBe(id);
				},
				{ timeout: 5000 },
			);
...
test("protection end reasons are presented as pauses rather than game wins or losses", () => {
	expect(reasonName("stagnation_loop")).toBe("费用保护：重复无果循环，已暂停");
	expect(reasonName("stagnation_no_apple")).toBe(
		"费用保护：长时间未吃苹果，已暂停",
	);
});
